...
## Standing exception: `next`

Next.js 14 no longer receives security backports — every advisory is patched
only on the 15.x line. While the app stays on 14.x, `yarn audit` reports a
growing set of `next` findings that **cannot** be resolved by any pin, because
`next` is a direct dependency. This is the single largest source of open
Dependabot alerts.

The CI audit gate therefore fails on `critical` only. Tighten it to
high + critical (`CODE & 24` in `.github/workflows/lintTest.yml`) as soon as the
Next 15 upgrade merges.
