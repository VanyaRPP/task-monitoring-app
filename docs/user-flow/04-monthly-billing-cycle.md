...
1.  **Select Domain**: The user chooses the Service Provider from a dropdown list.
    *   *API Interaction*: The dropdown is populated by a call to `GET /api/domain/admin`.

2.  **Select Street**: After selecting a Domain, this dropdown is populated with associated streets.
    *   *API Interaction*: `GET /api/filter/street?domainId=[selected_domain_id]`.
...
4.  **Select Company**: Finally, the user selects the specific tenant company.
    *   *API Interaction*: `GET /api/filter/real-estate?domainId=...&streetId=...`.
...
*   **Previous Readings Fetch**: The system queries for the most recent payment for this company in the *previous* month to retrieve the last meter readings.
    *   *API Interaction*: `GET /api/spacehub/payment?companyId=...&to=[start_of_current_month]`.
