...
export function generatePaymentsData(data) {
  return data.map((payment) => ({
    ...
    'Дата створення': styled(
      dateToDefaultFormat(payment.invoiceCreationDate),
      'Дата створення'
    ),
    Дебет: styled(
      payment.type === 'debit' && payment.generalSum != null
        ? formatNumber(payment.generalSum)
        : '-',
      'Дебет'
    ),
    Кредит: styled(
      payment.type === 'credit' && payment.generalSum != null
        ? formatNumber(payment.generalSum)
        : '-',
      'Кредит'
    ),
    'За місяць': styled(
      payment?.monthService?.date
        ? dayjs(payment.monthService.date).format('MMMM YYYY')
        : '-',
      'За місяць'
    ),
    Розміщення: styled(
      formatNumber(getInvoiceSum(payment, 'placingPrice')),
      'Розміщення'
    ),
    Утримання: styled(
      formatNumber(getInvoiceSum(payment, 'maintenancePrice')),
      'Утримання'
    ),
    'Індекс інфляції': styled(
      formatNumber(getInvoiceSum(payment, 'inflicionPrice')),
      'Індекс інфляції'
    ),
    Електропостачання: styled(
      formatNumber(getInvoiceSum(payment, 'electricityPrice')),
      'Електропостачання'
    ),
    Водопостачання: styled(
      formatNumber(getInvoiceSum(payment, 'waterPrice')),
      'Водопостачання'
    ),
    'Вивіз ТПВ': styled(
      formatNumber(getInvoiceSum(payment, 'garbageCollectorPrice')),
      'Вивіз ТПВ'
    ),
    Водонарахування: styled(
      formatNumber(getInvoiceSum(payment, 'waterPart')),
      'Водонарахування'
    ),
    Знижка: styled(formatNumber(getInvoiceSum(payment, 'discount')), 'Знижка'),
    Прибирання: styled(
      formatNumber(getInvoiceSum(payment, 'cleaningPrice')),
      'Прибирання'
    ),
    'Додаткові послуги': styled(
      formatNumber(
        (payment.invoice || []).find((i) => i.type === 'custom')?.price
      ),
      'Додаткові послуги'
    ),
  }))
