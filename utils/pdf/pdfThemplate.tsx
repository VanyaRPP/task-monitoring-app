...
export async function generateHtmlFromThemplate(
  ...
  if (isCredit) {
    const sum = paymentData?.generalSum ?? 0
    return `
    <!DOCTYPE html>
    <html lang="uk">
      <head>
        ...
        <title>Invoice</title>
        <style>${sharedStyles}
          .description-block { margin-top: 1.5rem; white-space: pre-wrap; }
        </style>
      </head>
      <body>
        <div class="invoice-wrapper">
          <div class="heading">${buildHeader(paymentData)}</div>
          <div class="description-block">${paymentData?.description || ''}</div>
          <div class="summary">
            <div>Загальна сума до оплати: <b>${sum} грн</b></div>
          </div>
        </div>
      </body>
    </html>`
