  ...
  useEffect(() => {
    if (consumed && total) {
      const rawPrice = Number(
        form.getFieldValue(['customServices', 5, 'price'])
      )

      const tariff = isVAT
        ? Number(rawPrice)
        : Number((Number(rawPrice) / 1.2).toFixed(8))
      ...
      const formatted = lossesPercent.toFixed(6)
      setLosses(formatted)
          ...
          <Space direction="vertical">
            ...
            <Text>
              ...
              {isVAT
                ? form.getFieldValue(['customServices', 5, 'price'])
                : (
                    form.getFieldValue(['customServices', 5, 'price']) / 1.2
                  ).toFixed(2)}
            </Text>
          </Space>
          <Space direction="vertical">
