...
const PriceList: FC<{ data: IPayment }> = ({ data }) => {
  ...
  const fileName = buildInvoiceFileName(payment as any, 'act')
  ...
  const { download, isLoading: pdfLoading } = usePdfDownload({
    fileName,
    onError: (msg) => message.error(isEnglish ? `PDF: ${msg}` : `PDF: ${msg}`),
  })
  ...
  return (
    <>
      ...
      <PrintDownloadMenu
        printLabel={isEnglish ? 'Print act' : 'Роздрукувати акт'}
        downloadLabel={isEnglish ? 'Save act PDF' : 'Зберегти акт PDF'}
        onPrint={handlePrint}
