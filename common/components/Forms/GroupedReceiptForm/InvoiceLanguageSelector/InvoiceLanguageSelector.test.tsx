...
describe('InvoiceLanguageSelector', () => {
  ...
  it('UA button has active class when lang=uk', () => {
    render(<InvoiceLanguageSelector lang="uk" onChange={() => {}} />)
    expect(screen.getByRole('button', { name: 'UA' }).className).toMatch(/active/)
    expect(screen.getByRole('button', { name: 'EN' }).className).not.toMatch(/active/)
  })
  ...
  it('EN button has active class when lang=en', () => {
    ...
    expect(screen.getByRole('button', { name: 'UA' }).className).not.toMatch(/active/)
  })
