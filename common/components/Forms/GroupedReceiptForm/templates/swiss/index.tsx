  ...
  <div className={sw.swissInvoice} ref={componentRef} style={{ width: '100%', margin: '2em auto 1em' }}>
    <div className={sw.swissHeader}>
      <div className={sw.swissBrand}>
        <div className={sw.swissBrandName}>{data?.reciever?.companyName || 'OLIMP DIGITAL OÜ'}</div>
    ...
    <div className={sw.swissMeta}>
      <div className={sw.swissMetaItem}>
        ...
        <span className={sw.swissMetaValue}>{dayjs(data?.invoiceCreationDate)?.format?.('DD.MM.YYYY')}</span>
      </div>
      <div className={sw.swissMetaItem}>
        <span className={sw.swissMetaLabel}>{isEnglish ? 'Due date' : 'Строк оплати'}</span>
        <span className={sw.swissMetaValue}>{dayjs(data?.invoiceCreationDate).add(5, 'd').format('DD.MM.YYYY')}</span>
      </div>
      <div className={sw.swissMetaItem}>
        ...
        {!!normalizedBankDetailsLines.length && (
          <div className={sw.swissBankCard}>
            {normalizedBankDetailsLines.map((line: string, idx: number) => {
              ...
              if (sep < 0) return <div className={sw.swissPartyLine} key={`bk-${idx}`}>{line}</div>
              return (
    ...
    <table className={sw.swissTable}>
      ...
      <tbody>
        {rows.map((item: any, index: number) => {
          ...
          const rate = Number.isFinite(Number(item?.price)) ? Number(item.price) : qty ? Number(item?.sum || 0) / qty : Number(item?.sum || 0)
          return (
    ...
    <div className={sw.swissFooter}>
      <div className={sw.swissThankYou}>{isEnglish ? 'Thank you' : 'Дякуємо'}</div>
      <div className={sw.swissSignatureBlock}>
        <div className={sw.swissSignatureLine}>______________</div>
        <div className={sw.swissSignatureCaption}>{data?.provider?.description?.split('\n')?.[0] || ''}</div>
      </div>
    </div>
  </div>
)
