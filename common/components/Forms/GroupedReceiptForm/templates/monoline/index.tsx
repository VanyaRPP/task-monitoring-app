  ...
  <div className={ml.mlInvoice} ref={componentRef} style={{ width: '100%', margin: '2em auto 1em' }}>
    <div className={ml.mlHeader}>
      <div className={ml.mlBrand}>
        <div className={ml.mlBrandName}>{data?.reciever?.companyName || 'OLIMP DIGITAL OÜ'}</div>
        <span className={ml.mlBrandRule} />
        <div className={ml.mlBrandSub}>Digital Services</div>
      </div>
      <div className={ml.mlInvoiceMeta}>
        <div className={ml.mlInvoiceWord}>{isEnglish ? 'Invoice' : 'Рахунок'}</div>
    ...
    <div className={ml.mlDateRow}>
      <div className={ml.mlDateItem}>
        ...
        <span className={ml.mlDateValue}>{dayjs(data?.invoiceCreationDate)?.format?.('DD.MM.YYYY')}</span>
      </div>
      <div className={ml.mlDateItem}>
        <span className={ml.mlDateLabel}>{isEnglish ? 'Due date' : 'Строк оплати'}</span>
        <span className={ml.mlDateValue}>{dayjs(data?.invoiceCreationDate).add(5, 'd').format('DD.MM.YYYY')}</span>
      </div>
      <div className={ml.mlDateItem}>
        ...
        {!!normalizedBankDetailsLines.length && (
          <div className={ml.mlBankBlock}>
            {normalizedBankDetailsLines.map((line: string, idx: number) => {
              ...
              if (sep < 0) return <div className={ml.mlBankLine} key={`bk-${idx}`}>{line}</div>
              return (
      ...
      <tbody>
        {rows.map((item: any, index: number) => {
          ...
          const rate = Number.isFinite(Number(item?.price))
            ? Number(item.price)
            : qty ? Number(item?.sum || 0) / qty : Number(item?.sum || 0)
          return (
    ...
    <div className={ml.mlTotalsArea}>
      <div className={ml.mlPaymentNote}>
        <div className={ml.mlPaymentNoteLabel}>{isEnglish ? 'Payment purpose' : 'Призначення платежу'}</div>
      ...
      <div className={ml.mlTotalsBlock}>
        ...
        <div className={`${ml.mlTotalRow} ${ml.mlGrandTotal}`}>
          ...
          <strong>{(+data?.generalSum || +data?.debit || total).toFixed(2)}&nbsp;{currencyLabel}</strong>
        </div>
      </div>
    </div>

    <div className={ml.mlFooter}>
      <div className={ml.mlFooterNote}>{isEnglish ? 'Thank you for your business' : 'Дякуємо за співпрацю'}</div>
      <div className={ml.mlSignatureBlock}>
        <div className={ml.mlSignatureLine}>______________</div>
        <div className={ml.mlSignatureCaption}>{data?.provider?.description?.split('\n')?.[0] || ''}</div>
      </div>
    </div>
  </div>
  )
