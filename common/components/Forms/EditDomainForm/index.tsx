...
export const EditDomainForm: React.FC<EditDomainFormProps> = ({
  ...
  const {
    data: { data: { 0: domain } } = { data: { 0: null } },
    ...
    async (fields: any) => {
      try {
        ...
        if ('data' in response) {
          ...
          // onFinish?.(response.data) // fix types
          onFinish?.(null)
...
const Streets: React.FC<EditFormAttributeProps> = ({
  ...
  // TODO: replace with useGetStreetsFiltersQuery()
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <Form.Item noStyle>
          {streets?.map((street) => {
            const streetData = allStreets?.find(({ _id }) => street === _id)
