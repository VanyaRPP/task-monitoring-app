...
export const scrollToFirstInvalidField = (page: Page) => {
  ...
  return {
    ...
    fillForm: async () => {
      ...
      await expect(page.locator('.ant-select[aria-label="Надавач послуг"] .ant-select-selector')).toHaveText('Pahan domen');
    },
    ...
    assertErrorDisplayed: async () => {
      ...
      const fieldWithError = page.locator('#payments_0_invoice_electricityPrice_sum');
