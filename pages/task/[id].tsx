...
import { useDeleteTaskMutation, useGetTaskByIdQuery } from '../../api/taskApi/task.api'
...
const Task: React.FC = () => {
  ...
  return (
    <div className={s.TaskContainer}>
      <Card
        className={s.Task}
        actions={session?.user?.email === user?.email ? [
          <Button ghost type='primary'>
            <EditOutlined  key="edit" />
          </Button>
          ,
          <Button ghost type='primary' onClick={() => taskDelete(task?._id)}>
